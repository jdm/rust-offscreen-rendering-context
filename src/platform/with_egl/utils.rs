...
fn create_pbuffer_surface(egl: &egl::Egl, display: EGLDisplay, config: EGLConfig, size: Size2D<i32>) -> Result<EGLSurface, &'static str> {
    let mut attrs = [
...
pub fn create_pixel_buffer_backed_offscreen_context(egl: egl::Egl,
                                                    size: Size2D<i32>,
    ...
    let surface = create_pbuffer_surface(&egl, display, config, size)?;

    NativeGLContext::new(egl, shared_with, display, surface, config, client_version)
}
